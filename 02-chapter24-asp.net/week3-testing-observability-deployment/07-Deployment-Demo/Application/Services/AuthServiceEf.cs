...
public class AuthServiceEf(UserManager<User> userManager, IConfiguration configuration) : IAuthService
{
  ...
  public async Task<MeResponseDto?> GetCurrentUserAsync(string userId)
  {
    ...
    return new MeResponseDto(user.Id, user.Email ?? "Email not provided", user.Name);
  }
}